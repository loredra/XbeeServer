...
int i = 0;
char msg[] = "Are there no one in Weimar borrows book anymore?";
char msgE2[] = "Come on, be the first one to borrow books today";
char msgE3[] = "Just 1 more book, and next level will be unlocked";
...
void loop() {
  ...
  if(content == "E2"){
      memcpy((uint8_t *)msgBuffer, (uint8_t *)msgE2, (sizeof(msgBuffer)));
  content="";  
  }
  if(content == "E3"){
      memcpy((uint8_t *)msgBuffer, (uint8_t *)msgE3, (sizeof(msgBuffer)));
  content="";  
  }
  if(content == "D1"){
    content="";
